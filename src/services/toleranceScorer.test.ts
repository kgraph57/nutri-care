...
function makeEntry(overrides: Partial<ToleranceEntry> = {}): ToleranceEntry {
  return {
    id: "entry-1",
    patientId: "patient-1",
    date: "2024-01-01",
...
describe("calculateToleranceScore — vomiting deductions", () => {
  ...
  it("deducts -3 for moderate vomiting", () => {
    const score = calculateToleranceScore(
      makeAssessment({ vomiting: "moderate" }),
    );
    expect(score).toBe(7);
  });
  ...
  it("deducts -5 for severe vomiting", () => {
    const score = calculateToleranceScore(
      makeAssessment({ vomiting: "severe" }),
    );
    expect(score).toBe(5);
  });
});
...
describe("calculateToleranceScore — abdominal distension deductions", () => {
  it("deducts -1 for mild distension", () => {
    const score = calculateToleranceScore(
      makeAssessment({ abdominalDistension: "mild" }),
    );
    expect(score).toBe(9);
  });
  ...
  it("deducts -2 for moderate distension", () => {
    const score = calculateToleranceScore(
      makeAssessment({ abdominalDistension: "moderate" }),
    );
    expect(score).toBe(8);
  });
  ...
  it("deducts -4 for severe distension", () => {
    const score = calculateToleranceScore(
      makeAssessment({ abdominalDistension: "severe" }),
    );
    expect(score).toBe(6);
  });
});
...
describe("calculateToleranceScore — bowel sounds deductions", () => {
  it("deducts -1 for reduced bowel sounds", () => {
    const score = calculateToleranceScore(
      makeAssessment({ bowelSounds: "reduced" }),
    );
    expect(score).toBe(9);
  });
  ...
  it("deducts -2 for absent bowel sounds", () => {
    const score = calculateToleranceScore(
      makeAssessment({ bowelSounds: "absent" }),
    );
    expect(score).toBe(8);
  });
});
...
describe("analyzeToleranceTrend — clinical scenarios", () => {
  it("patient recovering: worsening to improving over 5 days", () => {
    const entries = [
