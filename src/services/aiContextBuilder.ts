...
function buildMenuSummary(
  ...
  for (const item of menu.items) {
    lines.push(
      `  - ${item.productName} ${item.volume}mL × ${item.frequency}回/日`,
    );
  }
  ...
  const deficient = adequacy.details.filter(
    (d) => d.status === "deficient" || d.status === "low",
  );
...
function buildRuleBasedAnalysis(
  ...
  const interactions = checkDrugNutrientInteractions(
    patient.medications,
    [],
    nutritionType,
  );
  ...
  if (recommendations.length > 0) {
    ...
    for (const rec of recommendations) {
      ...
      for (const p of rec.products.slice(0, 3)) {
        lines.push(
          `    → ${String(p.product["製剤名"] ?? "不明")}: ${p.rationale}`,
        );
      }
    }
  }
  ...
  if (interactions.length > 0) {
    ...
    for (const inter of interactions) {
      lines.push(
        `  [${inter.severity}] ${inter.drug} ↔ ${inter.interaction}: ${inter.recommendation}`,
      );
    }
  }
...
function buildProtocolSummary(
  ...
  for (const step of protocol.steps.slice(0, 5)) {
    lines.push(
      `  Day${step.day}: ${step.rate}mL/h (${step.dailyVolume}mL/day) - ${step.percentOfTarget}%`,
    );
  }
...
export function buildNutritionContext(
  ...
  return {
    ...
    menuSummary: menu.requirements
      ? buildMenuSummary(menu, menu.requirements)
      : `【メニュー: ${menu.menuName}】\n栄養要件未設定`,
    ruleBasedAnalysis: buildRuleBasedAnalysis(
      patient,
      labData,
      nutritionType,
      products,
    ),
    protocolSummary: menu.requirements
      ? buildProtocolSummary(patient, nutritionType, menu.requirements)
      : "【投与プロトコル】\n栄養要件未設定",
  };
}
...
function buildUserPerformance(
  ...
  for (const item of menu.items) {
    lines.push(
      `  - ${item.productName} ${item.volume}mL × ${item.frequency}回`,
    );
  }
  return lines.join("\n");
}
...
export function buildClinicalSystemPrompt(context: NutritionContext): string {
  return `あなたは臨床栄養管理の専門AIアシスタントです。
