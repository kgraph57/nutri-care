...
export const calculateBasalMetabolicRate = (patient: Patient): number => {
  ...
  if (patient.gender === '男性') {
    bmr = 66.5 + (13.75 * patient.weight) + (5.003 * patient.height) - (6.775 * patient.age);
  } else {
    bmr = 655.1 + (9.563 * patient.weight) + (1.850 * patient.height) - (4.676 * patient.age);
  }
...
export const calculateTotalEnergyRequirement = (
  ...
  const activityFactor = ACTIVITY_FACTORS[activityLevel as keyof typeof ACTIVITY_FACTORS] || 1.0;
  const stressFactor = STRESS_FACTORS[stressLevel as keyof typeof STRESS_FACTORS] || 1.2;
...
export const calculateNutritionRequirements = (
  ...
  const totalEnergy = calculateTotalEnergyRequirement(patient, activityLevel, stressLevel);
