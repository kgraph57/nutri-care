  ...
  try {
    ...
    if (stored) {
      ...
      if (typeof parsed === "object" && parsed !== null) {
        return mergeWithSampleData(parsed as Record<string, ToleranceEntry[]>);
      }
    }
  } catch {
    // ignore
...
export function useToleranceData() {
  const [toleranceMap, setToleranceMap] =
    useState<Record<string, ToleranceEntry[]>>(loadFromStorage);
